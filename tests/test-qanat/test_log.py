"""Comprehensive tests for qanat.log module."""
...
from qanat.context import QanatContextFilter, get_qanat_context
from qanat.enums import ExecutionMode
from qanat.log import instrument_logging, logger
...
@pytest.fixture(autouse=True)
def reset_qanat_context():
    """Reset QanatContext before each test to avoid pollution."""
    # Import here to avoid circular dependency
    from qanat.context import _qanat_context

    # Clear the context before each test
    _qanat_context.set(None)
    yield
...
class TestModuleSetup:
    """Test qanat.log module setup."""

    def test_logger_has_qanat_context_filter(self):
        """Module logger has QanatContextFilter attached."""
        # Check that at least one filter is a QanatContextFilter
        has_context_filter = any(
            isinstance(f, QanatContextFilter) for f in logger.filters
        )
...
class TestLoggingIntegration:
    """Test logging integration with QanatContext."""
    ...
    def test_log_with_context_uses_real_values(self):
        ...
        # Apply filter
        qanat_filter = QanatContextFilter(default_value="DEFAULT")
