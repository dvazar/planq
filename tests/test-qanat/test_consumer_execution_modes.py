...
class TestThreadExecution:
    ...
    async def test_basic_execution_without_time_limit(
        self, thread_consumer: QanatConsumer, sync_handler
    ):
    ...
    async def test_execution_with_time_limit_completes_in_time(
        self, thread_consumer: QanatConsumer
    ):
    ...
    async def test_timeout_raises_handler_timeout(
        self, thread_consumer: QanatConsumer, slow_sync_handler
    ):
    ...
    async def test_timeout_calls_ctx_cancel(
        ...
        def handler_with_ctx_capture():
            ...
            captured_ctx = get_qanat_context()
            time.sleep(0.5)
    ...
    @pytest.mark.asyncio
    async def test_qanat_context_accessible_inside_thread(
        self, thread_consumer: QanatConsumer
    ):
        """QanatContext accessible via get_qanat_context() in THREAD handlers.
    ...
    async def test_thread_handler_returns_task_result_with_headers(
        ...
        """
        from qanat.models import TaskResult
    ...
    @pytest.mark.asyncio
    async def test_mixed_args_and_kwargs(self, thread_consumer: QanatConsumer):
    ...
    @pytest.mark.asyncio
    async def test_handler_with_no_params(self, thread_consumer: QanatConsumer):
...
class TestThreadContextCancellation:
    ...
    async def test_handler_checks_is_cancelled_flag_in_loop(
        self, thread_consumer: QanatConsumer, cancellation_aware_handler
    ):
...
class TestProcessPoolManagement:
    ...
    def test_process_pool_initialization(self):
        ...
        """
        from qanat.consumer import _ProcessPool
    ...
    def test_lock_protects_active_pids_access(self):
        ...
        futures_tasks = [pool.submit(_process_handler_sleeping, 0.5) for _ in range(5)]
    ...
    def test_process_pool_supports_positional_and_keyword_args(self):
        ...
        future, task_id = pool.submit(_process_handler_with_args_kwargs, 1, 2, x=3, y=4)
        result = future.result(timeout=2.0)
...
class TestProcessExecution:
    ...
    async def test_process_workers_none_raises_runtime_error(
        ...
        """
        with pytest.raises(RuntimeError, match="ProcessPoolExecutor not configured"):
    ...
    async def test_windows_with_time_limit_raises_feature_not_supported(
        self, process_consumer: QanatConsumer
    ):
        ...
        - Clear error message
        """
        with pytest.raises(FeatureNotSupportedError, match="process_time_limit.*Windows"):
    ...
    @pytest.mark.skipif(
        sys.platform == "win32", reason="Unix-only test (signals not supported on Windows)"
    )
    @pytest.mark.asyncio
...
@pytest.mark.skipif(sys.platform == "win32", reason="Signal handling requires Unix")
class TestProcessSignalHandling:
    ...
    def test_sigalrm_handler_raises_handler_timeout(self):
        ...
        """
        from qanat.consumer import _sigalrm_handler
    ...
    def test_sigterm_handler_raises_process_shutdown(self):
        ...
        """
        from qanat.consumer import _sigterm_handler
        from qanat.exceptions import ProcessShutdown
